...
export async function TopChannels({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        "grid rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 black:bg-gray-black black:shadow-card",
        className,
      )}
    >
      <h2 className="mb-4 text-body-2xlg font-bold text-black black:text-white">
        Top Channels
      </h2>
      ...
      <Table>
        ...
        <TableBody>
          {data.map((channel, i) => (
            <TableRow
              className="text-center text-base font-medium text-black black:text-white"
              key={channel.name + i}
            >
