...
export default function TableBasedView({
  group,
  index,
  lang,
  roman,
  ...
  const editQuestion = (text: string, groupId: string, questionId: string) => {
    const updated = questionList.map((group: any) => {
      if (group.id === groupId) {
        return {
          ...group,
          questions: group.questions.map(
            (q: any) => (q.id === questionId ? { ...q, table: text } : q), // ✅ fix here
          ),
        };
      }
      return group;
                ...
                <span className="w-full text-black">
                  ...
                  >
                    <Editor
                      ...
                      onChange={(e) => {
                        if (groupId)
                          editPassageQuestion?.("", e, group.id, q.id, groupId);
                        else editQuestion(e, group.id, q.id);
                      }}
                    />
                  </div>
                </span>
