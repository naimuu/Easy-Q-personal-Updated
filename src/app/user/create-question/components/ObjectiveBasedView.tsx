...
import { FaListOl, FaListUl, FaTrash } from "react-icons/fa";
...
import { getFormattedNumber, getAnyLabel, toBanglaNumber, toArabicNumber } from "./numberingUtils";
import { useQuestionContext } from "./QuestionContext";
import TextWithFractions from "./TextWithFractions";

export default function ObjectiveBasedView({
  group,
  index,
  editPassageOption,
  lang,
  ...
  const [optionNumberType, setOptionNumberType] = React.useState(
    lang === "bn"
      ? "bangla"
      : lang === "en"
        ? "english"
  ...
  const [localEditingOptionId, setLocalEditingOptionId] = useState<string | null>(null); // ✅ added for option
  ...
  useEffect(() => {
            ...
            questions: g.questions.map((i) => {
              if (i.id === group.id) {
                return {
                  ...
                  anyMode: anyMode,
                  mark: mark,
                  numbering: numberType,
                  optionNumbering: optionNumberType,
                };
              }
              return i;
            }),
    ...
    setQuestionList(updateList);
  }, [answer, anyMode, mark, numberType, optionNumberType]);
  ...
  const editOption = (text: string, groupId: string, questionId: string, optionId: string) => {
    const updated = questionList.map((group) => {
      if (group.id === groupId) {
        return {
          ...group,
    ...
    >
      ...
      >
              ...
              {mark && (
                <span className="font-mono dir-ltr whitespace-nowrap">
                  {(() => {
                    ...
                    const m = parseFloat(mark || "0");
                    const t = c * m;
                    // Determine language for display inside the editor (prefer anyMode, fallback to set lang)
                    // Note: Editor usually runs in English interface but we want to show the specific numeral type chosen
                    let targetLang = "en";
          ...
          {group.questions.map((q: any, qIndex: number) => {
            return (
              <div
                ...
                className={`group flex flex-col gap-2 rounded-md bg-gray-100 px-3 py-2 ${isArrangeMode && !disabled ? "hover:cursor-move" : "cursor-default"} hover:bg-gray-200 ${isRTL ? "text-rtl" : ""}`}
              >
                <div className="flex items-center justify-between">
                  <span
                    className="flex w-full items-center font-semibold gap-2 text-black"
                    onDoubleClick={() => {
                      if (disabled) {
                        onLockedFeature?.(
                          "Edit and Customize",
                ...
                <div
                  className={`grid justify-between gap-1 text-black ml-[18px] ${(() => {
                    const maxLen = Math.max(
                      ...(q?.options?.map((o: any) => (o.name || "").length) || [0]),
                    );
                    ...
                    <span
                      ...
                      onDoubleClick={() => {
                        if (disabled) {
                          ...
                          setLocalEditingOptionId(opt.id);
                        }
                      }}
                    >
                      {getFormattedNumber(i, optionNumberType)}{" "}
                      {
                        localEditingOptionId === opt.id ? (
                          <input
                            autoFocus
                            type="text"
                            value={opt?.name}
                            onBlur={() => setLocalEditingOptionId(null)}
                            onKeyDown={(e) => {
                              if (e.key === "Enter") setLocalEditingOptionId(null);
                            }}
                            onChange={(e) =>
                              groupId && editPassageOption
                                ? editPassageOption(
                                  e.target.value,
                                  group.id,
                                  q.id,
                                ...
                                : editOption(e.target.value, group.id, q.id, opt.id)
                            }
                            className="border-none bg-white focus:outline-none text-black rounded px-1"
                          />
                        ) : (
        ...
        <div className="mt-5 flex flex-wrap items-center justify-around gap-4 border-t pt-3">
          ...
          <Dropdown onChange={setNumberType}>
            <button title="Question Numbering">
              <FaListOl />
            </button>
          </Dropdown>
          <Dropdown onChange={setOptionNumberType}>
            <button title="Option Numbering">
              <FaListUl />
            </button>
          </Dropdown>
        </div>
